...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'save') {
        ...
        try {
            if ($id) {
                $stmt = $db->prepare("UPDATE courses SET name=?, description=?, category=?, image_path=?, teacher_id=?, duration_hours=?, status=? WHERE id=?");
                $stmt->execute([$name, $description, $category, $image_path ?: null, $teacher_id, $duration_hours, $status, $id]);
            } else {
                $stmt = $db->prepare("INSERT INTO courses (name, description, category, image_path, teacher_id, duration_hours, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
            ...
            <div class="column">
                <div class="level mb-6">
                    <div class="level-left">
                        <h1 class="title is-3">Каталог программ ДПО</h1>
                    </div>
                    ...
                    <div class="column is-6-tablet is-4-desktop is-3-widescreen">
                        <div class="card course-card admin-compact-card">
                            ...
                            <div class="card-content">
                                <span class="tag <?= $c['status'] === 'Активный' ? 'is-success' : 'is-warning' ?>">
                                    <?= htmlspecialchars($c['status']) ?>
                                </span>
                                <h3 class="title is-6 mt-2 mb-2"><?= htmlspecialchars($c['name']) ?></h3>
                                <p class="has-text-grey is-size-7">
                                    Вид программы: <strong><?= htmlspecialchars($c['category'] ?? 'Не указан') ?></strong>
                                </p>
                                <p class="has-text-grey is-size-7">
                                    Объём: <strong><?= (int)($c['duration_hours'] ?? 0) ?> ч.</strong>
                                </p>
                                ...
                                <div class="buttons mt-3">
                                    <button onclick='editCourse(<?= json_encode($c) ?>)' 
                                            class="button is-info is-light is-fullwidth">
                                        Редактировать
                                    </button>
                                    <button onclick="<?= $c['status']==='Архив' ? "restoreCourse({$c['id']})" : "archiveCourse({$c['id']})" ?>" 
                                            class="button <?= $c['status']==='Архив' ? 'is-success' : 'is-light' ?> is-fullwidth">
...
async function uploadCourseImageIfNeeded() {
    ...
    const response = await fetch('manage_courses.php?action=upload_image', {
        method: 'POST',
        body: formData
    });
...
async function saveCourse() {
    ...
    const res = await fetch('manage_courses.php?action=save', {
        method: 'POST',
